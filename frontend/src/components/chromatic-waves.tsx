...
/**
 * Chromatic Waves — a two-pass WebGL2 generative background:
 *   1. Perlin noise field rendered to an offscreen render target
 *      (flowing chromatic waves).
 *   2. A half-tone dot grid samples that texture and maps each cell's
 *      luminance to a palette color and dot radius.
 *
 * Adapted for the Pathshala login hero panel:
 *  - Transparent background (the panel's own dark gradient shows through).
 *  - Brand palette (mint/teal family, matching the login-hero accents).
 *  - Desktop-only (matchMedia min-width: 1024px, matching the lg: breakpoint
 *    the hero panel uses) so mobile never pays the GPU cost for a hidden canvas.
 *  - Animated at ~30fps with rAF and delta-time based uniforms.
 */
...
export default function ChromaticWaves({
  ...
  // 1024px the canvas is invisible — don't run a WebGL pipeline for it.
  ...
  useEffect(() => {
    ...
    // Draw an initial static frame so the panel isn't blank on first paint.
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [frequency, speed, effPaletteCount, paletteKey, cellSize, gamma, paletteBias]);
